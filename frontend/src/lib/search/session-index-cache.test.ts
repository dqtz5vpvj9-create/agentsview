...
function message(text: string): Message {
  const content = `\`\`\`text\n${text}\n\`\`\``;
  return { id: id++, session_id: "cache-test", ordinal: 0, role: "assistant", content,
    content_length: content.length, timestamp: "2026-01-01T00:00:00Z", has_thinking: false,
    thinking_text: "", has_tool_use: false, is_system: false, model: "", context_tokens: 0, output_tokens: 0 };
}
...
describe("prepared session block cache", () => {
  it("keeps exact positions across repeated query changes", () => {
    ...
    expect(buildSessionIndex([source], "needle").matches.map(({ start, end }) => [start, end])).toEqual([[0, 6], [9, 15]]);
    expect(buildSessionIndex([source], "i").matches.map(({ start, end }) => [start, end])).toEqual([[7, 8]]);
  ...
  it("invalidates when a replacement message version arrives", () => {
    const source = message("before"); buildSessionIndex([source], "needle");
  ...
  it("keeps blank queries empty even after a cache is populated", () => {
    const source = message("needle"); expect(buildSessionIndex([source], "needle").total).toBe(1);
