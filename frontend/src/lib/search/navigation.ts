...
export function sameCursor(a: SearchCursor, b: SearchCursor): boolean {
  return a.ordinal === b.ordinal && a.blockKey === b.blockKey &&
...
/** Resolve a stable tuple, falling forward from its message in display order. */
export function resolveSearchMatch(
  ...
  return ordered.find((match) =>
    newestFirst ? match.ordinal <= anchor : match.ordinal >= anchor,
  ) ?? null;
}
...
export function stepSearchMatch(
  ...
  const index = current
    ? ordered.findIndex((match) => sameCursor(match, current))
    : -1;
