...
function pane(scale = 1): HTMLElement {
  ...
  Object.defineProperties(element, {
    ...
    offsetWidth: { value: 204 }, offsetHeight: { value: 204 },
    scrollWidth: { value: 1500 }, scrollHeight: { value: 1500 },
  });
  vi.spyOn(element, "getBoundingClientRect").mockImplementation(() => ({
    x: 50 * scale, y: 30 * scale,
    left: 50 * scale, top: 30 * scale,
    right: 254 * scale, bottom: 234 * scale,
    width: 204 * scale, height: 204 * scale,
    toJSON() { return {}; },
  }));
  return element;
}
...
describe("scaled search geometry", () => {
  it.each([0.75, 1, 1.3, 2])("converts viewport deltas to layout offsets at scale %s", (scale) => {
    ...
    const scroll = vi.fn((offset: number) => { element.scrollTop = offset; });
    ...
    expect(revealInContainer(element, () => target(element, scale), true, true, scroll)).toBe(false);
    expect(scroll).toHaveBeenCalledTimes(1);
  });
  ...
  it.each([0.75, 1, 1.3, 2])("does not move an already visible match at scale %s", (scale) => {
    ...
    document.body.append(element);
    const scroll = vi.fn();
    expect(revealInContainer(element, () => target(element, scale, 150), true, true, scroll)).toBe(false);
...
describe("scrolling inside searchable Markdown blocks", () => {
  it("starts at a matched text node inside a descendant pane", () => {
    ...
    code.append(text); element.append(code); block.append(element); root.append(block);
  ...
  it("accepts an element boundary as well as a text boundary", () => {
    ...
    element.append(span); block.append(element); root.append(block); document.body.append(root);
  ...
  it("retains the block fallback when a live range is not yet available", () => {
    const root = document.createElement("div");
    const block = pane(); root.append(block); document.body.append(root);
  ...
  it("does not follow a stale range into another transcript", () => {
    ...
    root.append(block); document.body.append(root, foreign);
    expect(scrollNestedContainers(block, root, () => target(foreign), foreign.firstChild!)).toBe(false);
  ...
  it("ignores blocks outside the target transcript", () => {
    const root = document.createElement("div");
    const block = pane(); document.body.append(root, block);
