...
import {
  cursorFor, matchesInDisplayOrder, resolveSearchMatch, sameCursor, stepSearchMatch,
} from "./navigation.js";

function match(ordinal: number, block = "text:0", occurrence = 0): Match {
  return { ordinal, blockKey: `${ordinal}:${block}`, occurrence,
    start: occurrence * 7, end: occurrence * 7 + 6 };
}

const chronological = [
  match(1), match(1, "text:0", 1), match(1, "thinking:2"),
  match(8), match(8, "tool-output:0"), match(8, "tool-output:0", 1),
];
...
describe("session occurrence navigation", () => {
  ...
  it.each([false, true])("starts at the first displayed occurrence, newestFirst=%s", (newestFirst) => {
  ...
  it("anchors forward in the displayed direction", () => {
    ...
    expect(resolveSearchMatch(matchesInDisplayOrder(chronological, true), null, 4, true)?.ordinal).toBe(1);
  });

  it("does not select an occurrence beyond the opening anchor until navigation", () => {
    expect(resolveSearchMatch(chronological, null, 99, false)).toBeNull();
    expect(resolveSearchMatch(matchesInDisplayOrder(chronological, true), null, 0, true)).toBeNull();
  });
  ...
  it("falls forward when the cursor's message disappears in either direction", () => {
    ...
    expect(resolveSearchMatch(matchesInDisplayOrder(chronological, true), cursor, null, true)?.ordinal).toBe(1);
  });

  it.each([false, true])("next and previous are inverse complete cycles, newestFirst=%s", (newestFirst) => {
    const ordered = matchesInDisplayOrder(chronological, newestFirst);
