...
function scroller(scale: number) {
  ...
  Object.defineProperties(element, {
    clientHeight: { value: 200 }, clientWidth: { value: 300 },
    offsetHeight: { value: 210 }, offsetWidth: { value: 310 },
    clientTop: { value: 5 }, clientLeft: { value: 5 },
    scrollHeight: { value: 2000 }, scrollWidth: { value: 2000 },
  });
  element.getBoundingClientRect = () => ({
    top: 40, left: 20, bottom: 40 + 210 * scale, right: 20 + 310 * scale,
    width: 310 * scale, height: 210 * scale, x: 20, y: 40, toJSON: () => ({}),
  });
  return element;
}

describe("search reveal under CSS zoom", () => {
  it.each([0.67, 0.9, 1, 1.2, 1.3, 2])("converts viewport deltas to scroll coordinates at %s", (scale) => {
  ...
  it.each([0, -1, Number.NaN, Number.POSITIVE_INFINITY])("uses a finite fallback for invalid scale %s", (scale) => {
