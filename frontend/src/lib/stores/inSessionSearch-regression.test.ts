...
vi.mock("./ui.svelte.js", () => ({ ui: {
  selectedOrdinal: null, sortNewestFirst: false, selectOrdinal: vi.fn(), setFollowLatest: vi.fn(),
} }));

let id = 900000;
function message(ordinal: number, content: string): Message {
  return { id: id++, session_id: "find-regression", ordinal, role: "assistant", content,
    content_length: content.length, timestamp: "2026-01-01T00:00:00Z",
    has_thinking: false, thinking_text: "", has_tool_use: false, model: "",
    context_tokens: 0, output_tokens: 0, is_system: false };
}
...
function setup() {
  ...
  const source = reactiveSource({
    sessionId: "find-regression", messages: [message(5, "needle needle"), message(9, "target target 中文")],
    loading: false, hasOlder: false, loadingOlder: false, ensureOrdinalLoaded,
  });
...
describe("session find interaction regressions", () => {
  ...
  it("does not apply intermediate IME text or navigate until composition ends", async () => {
    const { store } = setup();
    await search(store);
    store.composing = true; store.query = "中文";
  ...
  it("exposes an incomplete-history result without retrying in a loop", async () => {
    ...
    await search(store); await settle();
  ...
  it("handles a synchronous loader failure and recovers on explicit retry", async () => {
    ...
    ensureOrdinalLoaded.mockImplementationOnce(() => { throw new Error("history unavailable"); });
    source.hasOlder = true;
    await search(store); await settle();
    expect(store.historyError).toBe(true);
  ...
  it("ignores a rejected request from the previous session", async () => {
    ...
    ensureOrdinalLoaded.mockImplementationOnce(() => new Promise<void>((_resolve, fail) => { reject = fail; }));
    source.hasOlder = true;
    await search(store);
    ...
    await tick();
    reject(new Error("old session failed")); await settle();
    expect(store.historyError).toBe(false);
