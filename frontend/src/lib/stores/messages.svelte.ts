...
export class MessagesStore {
  ...
  async ensureHistoryLoaded(): Promise<void> {
    const id = this.sessionId;
    const signal = this.abortController?.signal;
    const current = () => this.sessionId === id && this.abortController?.signal === signal && !signal?.aborted;
  ...
  private async loadRemainingMessages(id: string, signal: AbortSignal): Promise<void> {
    ...
    try {
      ...
      for (;;) {
        ...
        const res = await withAbort(
          SessionsService.getApiV1SessionsIdMessages({
            id, from, limit: MESSAGE_PAGE_SIZE, direction: "asc",
          }) as unknown as Promise<MessagesResponse>, signal,
        );
        if (!current()) return;
        if (res.messages.length === 0) {
          this.historyComplete = !this.hasOlder;
