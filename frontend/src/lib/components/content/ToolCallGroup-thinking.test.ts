...
function message(ordinal: number, structured: boolean): Message {
  ...
  return { id: id++, session_id: "group-thinking", ordinal, role: "assistant", content,
    content_length: content.length, timestamp: "2026-01-01T00:00:00Z", has_thinking: true,
    thinking_text: "", has_tool_use: true, is_system: false, model: "", context_tokens: 0, output_tokens: 0,
    tool_calls: structured ? [{ tool_name: "Bash", category: "Bash", result_content: "ordinary output" }] : undefined };
}
beforeEach(() => {
  vi.useFakeTimers(); inSessionSearch.close(); inSessionSearch.clearQuery(); messages.clear();
  messages.sessionId = "group-thinking"; messages.historyComplete = true;
  ui.selectedOrdinal = null; ui.sortNewestFirst = false; ui.visibleBlocks = new Set(["user", "assistant", "tool"]);
});
afterEach(async () => {
  if (component) await unmount(component); component = undefined;
  inSessionSearch.close(); inSessionSearch.clearQuery(); messages.clear();
  ui.showAllBlocks(); ui.sortNewestFirst = false; document.body.replaceChildren(); vi.useRealTimers();
});
async function render(structured: boolean, newestFirst: boolean) {
  ...
  component = mount(ToolCallGroup, { target: document.body, props: {
    messages: messages.messages, timestamp: "2026-01-01T00:00:00Z", searchable: true, sortNewestFirst: newestFirst,
  } });
  await tick(); expect(document.querySelector(".thinking-header")).toBeNull();
  inSessionSearch.open(); inSessionSearch.query = "needle";
...
describe("thinking inside tool groups", () => {
  it.each([[true, false], [false, false], [true, true], [false, true]])(
    "reveals indexed thinking for structured=%s newestFirst=%s", async (structured, newestFirst) => {
  ...
  it("keeps manual collapse until the next occurrence is selected", async () => {
    ...
    const header = document.querySelector<HTMLButtonElement>('[data-message-ordinal="7"] .thinking-header')!;
    header.click(); await tick(); expect(document.querySelector(".thinking-content")).toBeNull();
