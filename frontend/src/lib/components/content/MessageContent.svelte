<script lang="ts">
  ...
  let showText = $derived(hasSearchQuery || ui.isBlockVisible(isUser ? "user" : "assistant"));
  ...
  async function handleForkFromHere() {
    ...
    try {
      ...
      const resp = await SessionsService.postApiV1SessionsIdResume({
      ...
      {:else}
        {#if showText}
          <div class="text-content markdown" {@attach searchBlock(searchKey)}
            use:highlightCodeFences={{ content: segment.content }}>
...
<style>
  ...
  .markdown :global(pre) { background: var(--code-bg); color: var(--code-text); border-radius: var(--radius-md); padding: 12px 16px; overflow-x: auto; margin: 0.5em 0; }
